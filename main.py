...
map_obj = map.Map(
    tile_imgs=r.tile_imgs,
    weights=weights,
    map_tile_width=p.MAP_TILE_WIDTH,
...
game_window = window.GameWindow(
    ...
    max_width=map_obj.width,
    max_height=map_obj.height,
    min_size=p.TILE_SIZE * 3,
    icon=r.player_image,
    tile_size=p.TILE_SIZE,
...
interface_layers = [pyglet.graphics.Group(order=x) for x in (2, 3)]

game_hud = hud.HUD(
    hud_batch=game_batch,
    hud_groups=interface_layers,
    inv_slot_img=r.inventory_slot,
    inv_select_img=r.inventory_select,
    inv_current_img=r.inventory_selected,
    middle=game_window.half_width,
    ...
    for j in range(p.MAP_TILE_WIDTH):
        ...
        tile_objs.append(
            map_object.MapObject(
                img=tile,
                traversable=traversability[tile],
                map_x=x,
                map_y=y,
                batch=None,
                group=background,
...
for _ in range(3 * p.MAP_TILE_SCALE):
    ...
    x, y = generate_position()
    ai_characters.append(
        char_fire.CharFire(map_x=x, map_y=y, group=foreground, batch=game_batch)
    )
    x, y = generate_position()
    ai_characters.append(
        char_green.CharGreen(map_x=x, map_y=y, group=foreground, batch=game_batch)
    )
...
for handler in [
    item
    for row in [getattr(obj, "event_handlers") for obj in (protagonist, game_hud)]
    for item in row
]:
...
cam.activate_tiles(
    tile_objs, range(*cam.in_view_i_range), range(*cam.in_view_j_range), True
)
...
def update(dt):
    if game_window.overlay:
        ...
        if protagonist.effect:
            try:
                new_ani = effect.Effect(
                    img=r.attack_animations[protagonist.current],
                    affinity=attack_affinities[protagonist.current],
                    x=protagonist.effect_x,
                    y=protagonist.effect_y,
                    group=foreground,
                    batch=game_batch,
                )
        ...
        for dy_obj in [protagonist] + ai_characters:
            ...
            dy_obj.check_traversability(
                tile_objs, env_obj_dict, max_width_env_imgs, max_height_env_imgs
            )
        ...
        for object_ in (
            cam.active_tiles + list(env_obj_dict.values()) + ai_characters + animations
        ):
            cam.apply(object_)
