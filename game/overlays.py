...
class Menu(Overlay):
    def __init__(self, game_window, title):
        self.items = []
        self.font_size = 36
        self.title_text = pyglet.text.Label(title, x=game_window.width // 2, y=game_window.height - self.font_size,
...
class MenuItem:
    def __init__(self, game_window, label, v_pos, activate_func):
        ...
        self.font_size = 14
        self.text = pyglet.text.Label(label, x=self.mid_x, y=self.mid_y, font_size=self.font_size,
                                      anchor_x='center', anchor_y='center')
        self.width = 100
        self.height = self.font_size * 2
        self.x = self.mid_x - self.width // 2
        self.y = self.mid_y - self.height // 2
        self.default_colour = (0, 0, 255,)
    ...
    def draw(self, selected):
        ...
        vertex_list = pyglet.graphics.vertex_list(
            num_vertices,
            ('v2i', (self.x, self.y + self.height,
                     self.x, self.y,
                     self.x + self.width, self.y + self.height,
                     self.x + self.width, self.y,)),
            ('c3B', self.default_colour * num_vertices)
        )

        if selected:
            vertex_list.colors = self.selected_colour * num_vertices

        vertex_list.draw(pyglet.gl.GL_TRIANGLE_STRIP)
...
class MainMenu(Menu):
    def __init__(self, game_window):
        ...
        self.separation = 50
        self.v_current = 100
        self.items.append(MenuItem(game_window, 'Play', self.v_current, game_window.play))
